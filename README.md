...
shell，那一行结论对你实际敲命令的地方不成立。现在先在「配置终端」里选择 bash、zsh 或 fish，
只显示所选终端的状态和**「自动配置」**按钮，不必把其他终端也配好。安装路径、完整 PATH 诊断、
手动配置和移除操作收在默认折叠的「高级选项」里；错误和路径冲突仍直接显示：

| shell | 自动配置做什么 | 移除 |
|---|---|---|
| zsh | 在 `~/.zshrc` 里维护一个带标记的块（PATH + 补全加载） | 有「移除」按钮，只删块 |
| bash | 同上，写入已有的首个登录配置文件；没有时创建 `~/.bash_profile` | 有「移除」按钮 |
| fish | 保存为 universal 变量，并验证新启动的 fish；不改启动文件 | 不自动移除已有路径 |

fish 配置会保留已有持久路径；重复点击不会叠加。配置时显示进度，失败原因显示在 fish 行旁。
检测超时或启动失败会显示「无法确认」，而不是误报「未配置」。

**对于 bash / zsh，GitPic 只在自己那对标记之间写字**，块外一个字节都不动：
