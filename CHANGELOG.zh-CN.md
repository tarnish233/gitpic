...
- 命令行区域改为原生分段选择器，只呈现一个终端的状态，不再并列三套说明。正常的 PATH 详情
  和补全信息移入默认折叠的高级选项；路径冲突、探测失败和配置错误继续在主卡片显示。
- 只展示所选终端的文件写入与备份提示。替换文件、重新指向链接和移除命令的确认保持不变，
  fish 的共享持久路径仍不提供自动移除操作。
- 刷新保留有效的终端选择，目标消失时才回退到可用登录终端；配置中返回通用页时优先展示
  正在处理的终端。新增选择逻辑和界面接线回归，使用真实 SwiftUI 组件检查多种状态与窗口宽度。
...
- 更新检查改用你已登录的 GitHub 凭据，额度从每小时 60 次（同一出口所有人共用）提到 5000 次
- 没登录过的不受影响；凭据过期时会自动退回匿名检查，不会因此报错
...
- 更新检查失败时会说清是限流还是别的原因，不再把每个 403 都当成限流
- 真的被限流时会告诉你还要等多久
- 设置里的「上次检查」改为「上次成功检查」，不再和下面的失败信息混读
...
- 更新窗口开着时，注销、重启、从 Dock 图标退出不再被拒绝（此前只能强制退出）
- 安装中途退出同样不会留下已挂载的磁盘映像、半成品副本和下载文件
