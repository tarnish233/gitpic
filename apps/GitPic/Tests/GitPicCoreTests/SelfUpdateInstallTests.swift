...
struct SelfUpdateInstallTests {
    ...
    /// `Task.checkCancellation` and a hop onto the staging queue in between.
    ...
    /// A command-line link points into the bundle by path, so replacing the bundle at that same
    /// path must make the command run the new embedded executable without touching the link.
    /// Running the link before and after the swap proves more than merely checking that it still
    /// resolves: the terminal must actually see the new version.
    @Test("the command-line link follows the swapped bundle")
    func swapKeepsTheCommandLineLink() throws {
        let f = try Self.fixture()
        defer { try? FileManager.default.removeItem(at: f.root) }

        let bin = f.root.appendingPathComponent("home/.local/bin", isDirectory: true)
        try FileManager.default.createDirectory(at: bin, withIntermediateDirectories: true)
        let binLink = bin.appendingPathComponent("gitpic")
        try FileManager.default.createSymbolicLink(
            at: binLink,
            withDestinationURL: f.target.appendingPathComponent("Contents/Resources/gitpic"))
        ...
        #expect(String(decoding: after.stdout, as: UTF8.self).trimmingCharacters(
            in: .whitespacesAndNewlines) == "0.19.0",
                "the command-line link still resolves to the old bundle after the swap")
    }
