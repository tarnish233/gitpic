...
mod tests {
    ...
    /// Read one whole HTTP request: the headers, then exactly the body
    /// `Content-Length` promises.
    ///
    /// **One `read` is not enough**, and that is worth spelling out because the single
    /// `sock.read` this replaced looked entirely reasonable. TCP is a stream, not a
    /// message queue: `reqwest` writes the headers and the body as separate segments, so
    /// one `read` frequently returns the headers alone. Measured before this fix,
    /// `put_file_sends_the_existing_sha_when_overwriting` failed about two runs in five.
    ///
    /// It stayed hidden because that is the only test here asserting on a request *body*.
    /// Every other assertion is against the request line or a header, which are always in
    /// the first segment — so the bug was invisible to all of them, and to that one too
    /// on any run that happened to pass.
